...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\hline
input & string & $T_{28 \times 28}$ where $T_{i,j} \in \{0, \ldots, 255\} $ & DimensionsError, FormatError \\
\hline
\end{tabular}
\end{center}
...
\begin{itemize}
  \item $n_{min}$: minimum number of pixels in the vertical direction
  \item $n_{max}$: maximum number of pixels in the vertical direction
  \item $m_{min}$: minimum number of pixels in the horizontal direction
  \item $m_{max}$: maximum number of pixels in the horizontal direction
  \item supportedFormats: list of supported image formats (e.g. PNG, JPEG)
\end{itemize}
...
\noindent input(path):
\begin{itemize} 
\item output: image := preprocessing(inputImage)
...
parameters. A FormatError exception is thrown if the input image is not one of
