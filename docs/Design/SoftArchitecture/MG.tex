...
\item [\refstepcounter{mnum} \mthemnum \label{mAccuracy}:] Accuracy Metrics Module
\end{description}
...
application. Note that training and testing the model is done in
\mref{mTraining} and \mref{mTesting} respectively, and is a separate routine
...
and data that the model was not trained on. This tests \mref{mModel} to compare
its accuracy to the previous OAR project.
\item[Implemented By:] \progname{}
\item[Type of Module:] Abstract Data Type
\end{description}
...
R3 & \mref{mTraining}, \mref{mTesting}, \mref{mModel}, \mref{mAccuracy}\\
