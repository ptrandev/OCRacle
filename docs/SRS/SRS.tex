...
\\
$P_{1 \times L}$ & N/A & Probability vector representing the output of the
...
\refstepcounter{theorynum}\label{TM:ReLU}
\noindent
\deftheory
% #2 refname of theory
{TM:ReLU}
...
\refstepcounter{theorynum}\label{TM:SM}
\noindent
\deftheory
% #2 refname of theory
{TM:SM}
...
\hline
\rowcolor[gray]{0.9}
Number& GD\refstepcounter{defnum}\thedefnum \label{GD_NN}\\
\hline
Label &\bf Neural Network\\
\hline
SI Units& N/A\\
\hline
Equation&
...
\hline
Description &
A neural network consists of multiple layers of nodes. The input layer takes in the
image data $I_{n \times m}$. The hidden layer applies a linear transformation
$W_1 x + b_1$ to the input data, followed by a non-linear activation function
(ReLU). The output layer applies the softmax function to the output of the
hidden layer to produce a probability distribution $\hat{y}$ over the
characters. The weights $W_1$ and biases $b_1$ are learned during training.
\\
\hline
...
\begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
  ...
  \hline
  Output & Trained parameters $\theta_t$\footnote{Such that, at time $t$, the
  ...
  updates the parameters to minimize the cross-loss entropy function. The
  hyperparameters $\beta_1$, $\beta_2$, $\alpha$, and $\epsilon$ control the
...
\begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
  ...
  the highest probability in $P$, where ties are broken by choosing the first
...
\item[R\refstepcounter{reqnum}\thereqnum \label{R_Inputs}:] (Prediction) The program accepts
...
\item[R\refstepcounter{reqnum}\thereqnum \label{R_OutputInputs}:] (Prediction) The input
...
\item[R\refstepcounter{reqnum}\thereqnum \label{R_Calculate}:] (Training) The \progname{}
...
\item[R\refstepcounter{reqnum}\thereqnum \label{R_VerifyOutput}:] (Training) The model
...
\item[R\refstepcounter{reqnum}\thereqnum \label{R_Output}:] (Prediction) The program outputs
...
\begin{tabular}{|c|c|c|c|c|c|c|c|c|c|}
  \hline
  & TM\ref{TM:BI} & TM\ref{TM:N} & TM\ref{TM:ADAM} & TM\ref{TM:CELF} & TM\ref{TM:ReLU} & TM\ref{TM:SM} & GD\ref{GD_NN} & IM\ref{IM_Training} & IM\ref{IM_PreProcessing} \\ \hline
...
\begin{tabular}{|c|c|c|c|c|c|c|c|c|c|}
  ...
  TM\ref{TM:ReLU} &     &     &         &         &        &      & X     &             &                  \\ \hline
  TM\ref{TM:SM} &     &     &         &         &        &      & X     &             &                  \\ \hline
  GD\ref{GD_NN} &     &     &         &         &        &      &       & X           &                  \\ \hline
